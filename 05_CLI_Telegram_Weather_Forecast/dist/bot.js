...
bot.onText(/With a (\d+)-hour interval/, async (msg, match) => {
    ...
    const lat = 48.4647;
    const lon = 35.0462;
    const apiUrl = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${process.env.Weather_Api_Key}`;
    try {
        const response = await axios.get(apiUrl);
        const forecasts = response.data.list;
        const filteredForecasts = forecasts.filter((_, index) => index % (interval / 3) === 0);
        let message = "";
